...
> [!NOTE]
> **最新发布** — `ClawPanel Pro v5.3.5` 与 `ClawPanel Lite v0.2.5` 已发布；本次重点修复 Lite 发布打包链路，在保留本机镜像与 `CLAWPANEL_PUBLIC_BASE` 的基础上，把 Lite 内嵌 OpenClaw runtime 提升到 `2026.4.5`，并在镜像源缺包时自动回退官方 npm。若一键安装/手动更新没有拿到最新版，请优先重新拉取安装脚本或检查本机更新镜像是否已同步。
