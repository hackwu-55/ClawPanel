...
> [!WARNING]
> **macOS，Windows用户** — 目前优先进行 Linux 版本开发和维护，macOS 和 Windows 用户也会逐步跟进，耐心等待～如果 macOS Windows 上遇到 bug，请通过 [Issues](https://github.com/zhaoxinyi02/ClawPanel/issues) 反馈，我们会统一处理。感谢理解！
...
> 📱 **扫码加入企微群**
> 
> <img src="img/wecom.png" width="300"/>
> 
...
- Bindings 路由规则管理：按顺序匹配、启停、重排与可视化编辑
- 路由预览器：输入 `channel/sender/peer/guild/team/account` 快速验证命中 Agent
- 会话管理支持 `agent=all` 聚合视图，定时任务支持选择 `sessionTarget`
- 支持回滚开关：`LEGACY_SINGLE_AGENT=true` 可退回单 Agent 兼容模式
...
> ⚠️ macOS 支持说明：目前开发者暂无 Mac 设备，近期会购入用于测试。macOS 用户遇到问题请提交 [Issue](https://github.com/zhaoxinyi02/ClawPanel/issues)。
