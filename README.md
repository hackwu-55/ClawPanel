...
> [!NOTE]
> **最新发布** — `ClawPanel Pro v5.3.6` 与 `ClawPanel Lite v0.2.6` 已发布；本次重点补修 Lite 发布脚本在 CI 中的函数定义顺序问题，继续保持 Lite 内嵌 OpenClaw runtime `2026.4.5`、本机镜像与 `CLAWPANEL_PUBLIC_BASE` 方案不变。若一键安装/手动更新没有拿到最新版，请优先重新拉取安装脚本或检查本机更新镜像是否已同步。
