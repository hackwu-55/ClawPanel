...
> [!NOTE]
> **最新发布** — `ClawPanel Pro v5.3.7` 与 `ClawPanel Lite v0.2.7` 已发布；本次新增腾讯官方微信 `openclaw-weixin` 通道面板接入，支持在渠道页直接生成登录二维码、轮询扫码状态、刷新二维码、查看已登录账号并执行退出登录。腾讯返回的是登录链接而不是图片，面板现已改为本地重新渲染二维码，避免浏览器直接加载失败。详情见 [changelogs/v5.3.7.md](changelogs/v5.3.7.md) 与 [changelogs/lite-v0.2.7.md](changelogs/lite-v0.2.7.md)。
...
| OpenClaw | 内置 `2026.4.5`，安装后直接可用 | 不内置，支持一键安装或外部接管 |
| 运行时控制 | 面板全托管 | 用户可自行决定 |
