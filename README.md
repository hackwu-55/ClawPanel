...
> [!NOTE]
> **最新发布** — `ClawPanel Pro v5.3.4` 与 `ClawPanel Lite v0.2.4` 已发布；本次重点把更新、安装、卸载与修复脚本统一收口到本机镜像与 `CLAWPANEL_PUBLIC_BASE`，并为面板自更新补齐 `checksums.txt + SHA256` 强校验。若一键安装/手动更新没有拿到最新版，请优先重新拉取安装脚本或检查本机更新镜像是否已同步。
